...
if __name__ == '__main__':
    ...
    r_s = np.array( [(i,(1/10**i)/10) for i in range(6)] )
    ...
    for dz in dz_s:
        for ir, r in r_s:
            ...
            print('z_test_v shape:',z_test_v.shape)
            key, *skeys = random.split(key,num_instances+1)
            # vectorize vmap over sh5,sh15; vectorize over model instances
            vmiObs = vmap(
            ...
            print('sh shapes')
            print( tree_util.tree_map( lambda a:a.shape, sh_out) )
            
            print('')
            print(f'run , dz: {run} , {dz}, {ir}')
            ...
            rpath_relative = ('results',experiment,f'run_dz{int(dz)}_ir{int(ir)}')
